...
const Navbar = () => {
  ...
  return (
    ...
    className={`font-poppins font-normal cursor-pointer text-[16px] hover:text-secondary ${
      active === nav.id ? 'text-secondary' : 'text-white'
    } ${index === navLinks.length - 1 ? 'mr-10' : 'mr-10'}`}
    onClick={() => {
      setActive(nav.id);
      router.push(`/${nav.id}`); // Routing via Next.js router
      console.log(`Navigating to dashboard: ${nav.title}`);
      // console.log(`current page: ${nav.title}`);
    }}
  >
    {nav.title}
  </li>
        ...
        >
          <ul className="list-none flex justify-end items-start flex-1 flex-col">
            {navLinks.map((nav, index) => (
              ...
              >
                <a href={``} onClick={() => navigate.push("/dashboard")}>
                  {nav.title}
                </a>
